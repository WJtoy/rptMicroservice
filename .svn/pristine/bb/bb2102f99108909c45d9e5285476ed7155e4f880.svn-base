...
public class OrderQtyDailyChartTasks {

    @Autowired
    private OrderQtyDailyChartService orderQtyDailyChartService;

    @Scheduled(cron = "${rpt.jobs.writeOrderQtyDailyChartJob}")
    public void autoWriteOrderQtyDailyChartJob() {
        if (!RptCommonUtils.scheduleEnabled()) {
            return;
        }
        //写入前一天数据
        Date date = DateUtils.addDays(new Date(), -1);
        orderQtyDailyChartService.saveOrderQtyDailyToRptDB(date);

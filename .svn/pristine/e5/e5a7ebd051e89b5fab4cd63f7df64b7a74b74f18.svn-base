...
import com.kkl.kklplus.provider.rpt.mapper.CustomerSpecialChargeAreaRptMapper;
...
public class CustomerSpecialChargeAreaRptService {

    @Resource
    private CustomerSpecialChargeAreaRptMapper customerSpecialChargeAreaRptMapper;
    ...
    public List<RPTSpecialChargeAreaEntity> getOldSpecialChargeData(Date startDate, Date endDate){
        List<RPTSpecialChargeAreaEntity> oldSpecialDataList = customerSpecialChargeAreaRptMapper.getOldSpecialData(startDate, endDate);
     ...
     * @param queryDate
     */
    public void insertCustomerSpecialChargeAreaRpt(Date queryDate){
        ...
        List<Map<String,Long>> countyIds = customerSpecialChargeAreaRptMapper.getCountyIds(Integer.parseInt(yearMonth),systemId);
        Map<String,Long> countyMap = new HashMap<>();
        for (Map<String,Long> map:countyIds) {
            countyMap.put(StringUtils.join(map.get("countyId"),":",map.get("productCategoryId"),":",map.get("customerId")),map.get("id"));
        }
        for (RPTSpecialChargeAreaEntity entity:oldSpecialChargeData) {
            ...
            String key = StringUtils.join(entity.getCountyId(), ":", entity.getProductCategoryId(),":",entity.getCustomerId());
            if (countyMap.get(key)!=null){
                entity.setId(countyMap.get(key));
                customerSpecialChargeAreaRptMapper.updateSpecialChargeRpt(entity);
            }else {
                customerSpecialChargeAreaRptMapper.insertSpecialChargeRpt(entity);
            }
//
        }
    }
    ...
    public List<RPTSpecialChargeAreaEntity> getSpecialChargeNewList(RPTSpecialChargeSearchCondition searchCondition){
        ...
        int systemId = RptCommonUtils.getSystemId();
        List<RPTSpecialChargeAreaEntity> specialChargeAreaList = customerSpecialChargeAreaRptMapper.getSpecialChargeAreaList(searchCondition.getYearmonth(),systemId, searchCondition.getProductCategoryIds(),searchCondition.getCustomerId(),searchCondition.getQuarter());
        Map<Long, RPTArea> countyMap = areaCacheService.getAllCountyMap();
        Map<Long, RPTArea> cityMap = areaCacheService.getAllCityMap();
    ...
    public boolean hasReportData(String searchConditionJson) {
        ...
        if (searchCondition != null && searchCondition.getYearmonth()!=null &&searchCondition.getYearmonth()!=0) {
            Integer rowCount = customerSpecialChargeAreaRptMapper.hasReportData(searchCondition.getYearmonth(),
                    RptCommonUtils.getSystemId(),searchCondition.getProductCategoryIds(),searchCondition.getCustomerId(),searchCondition.getQuarter());
            result = rowCount > 0;
        }
        return result;
    }
    ...
    public void deleteSpecialChargeFromRptDB(Integer selectYear,Integer selectMonth) {
        ...
        int systemId = RptCommonUtils.getSystemId();
        customerSpecialChargeAreaRptMapper.deleteSpecialChargeRpt(systemId, StringUtils.toInteger(yearMonth));
            ...
            try {
                ...
                while (beginDate.getTime() < endDate.getTime()) {
                    switch (operationType) {
                        case INSERT:
                            insertCustomerSpecialChargeAreaRpt(beginDate);
                            break;
...
//                            insertCustomerSpecialChargeAreaRpt(beginDate);
                ...
                log.error("CustomerSpecialChargeAreaRptService.rebuildMiddleTableData:{}", Exceptions.getStackTraceAsString(e));
            }
        }
        return result;
    }
...
//     * @return
//     */
//    public SXSSFWorkbook exportCustomerSpecialChargeCityRpt(String searchConditionJson, String reportTitle) {
...
//            }
//        }catch (Exception e) {
//            log.error("KA省市特殊费用分布报表写入excel失败:{}",Exceptions.getStackTraceAsString(e));
//            return null;
//        }
//        return xBook;
//    }
     ...
     * @return
     */
    public SXSSFWorkbook exportCustomerSpecialChargeByCounty(String searchConditionJson, String reportTitle) {
        ...
        try {
            ...
            log.error("KA区县特殊费用分布报表写入excel失败:{}",Exceptions.getStackTraceAsString(e));
            return null;
        }
        return xBook;
    }

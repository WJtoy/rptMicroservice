...
public class UncompletedOrderRptService extends RptBaseService {
...
//                    orderItems = RPTOrderItemUtils.fromOrderItemsJson(item.getOrderItemJson());
    ...
    public SXSSFWorkbook UncompletedOrderExport(String searchConditionJson, String reportTitle) {
        ...
        try {
            ...
            ExportExcel.createCell(headerFirstRow, 17, xStyle, ExportExcel.CELL_STYLE_NAME_HEADER, "完成时间");
            xSheet.addMergedRegion(new CellRangeAddress(headerFirstRow.getRowNum(), headerFirstRow.getRowNum() + 1, 17, 17));

            ExportExcel.createCell(headerFirstRow, 18, xStyle, ExportExcel.CELL_STYLE_NAME_HEADER, "客户货款");
            xSheet.addMergedRegion(new CellRangeAddress(headerFirstRow.getRowNum(), headerFirstRow.getRowNum(), 18, 19));

            ExportExcel.createCell(headerFirstRow, 20, xStyle, ExportExcel.CELL_STYLE_NAME_HEADER, "状态");
            xSheet.addMergedRegion(new CellRangeAddress(headerFirstRow.getRowNum(), headerFirstRow.getRowNum() + 1, 20, 20));

            ExportExcel.createCell(headerFirstRow, 21, xStyle, ExportExcel.CELL_STYLE_NAME_HEADER, "停滞原因");
            ...
            ExportExcel.createCell(headerSecondRow, 18, xStyle, ExportExcel.CELL_STYLE_NAME_HEADER, "派单金额");
            ExportExcel.createCell(headerSecondRow, 19, xStyle, ExportExcel.CELL_STYLE_NAME_HEADER, "冻结金额");
            ...
            if (list != null) {
                for (int i = 0; i < list.size(); i++) {
                    ...
                    ExportExcel.createCell(dataRow, 18, xStyle, ExportExcel.CELL_STYLE_NAME_DATA, orderMaster.getExpectCharge());
                    if (rowSpan > 0) {
                    ...
                    ExportExcel.createCell(dataRow, 19, xStyle, ExportExcel.CELL_STYLE_NAME_DATA, orderMaster.getBlockedCharge());
                    if (rowSpan > 0) {
                    ...
                    ExportExcel.createCell(dataRow, 20, xStyle, ExportExcel.CELL_STYLE_NAME_DATA, orderMaster.getStatus()==null ? "" :orderMaster.getStatus().getLabel());
                    if (rowSpan > 0) {
                        xSheet.addMergedRegion(new CellRangeAddress(dataRow.getRowNum(), dataRow.getRowNum() + rowSpan, 20, 20));
                    }
                    ExportExcel.createCell(dataRow, 21, xStyle, ExportExcel.CELL_STYLE_NAME_DATA, orderMaster.getPendingType() == null ? "" : orderMaster.getPendingType().getLabel());
                    if (rowSpan > 0) {
                ...
                ExportExcel.createCell(dataRow, 18, xStyle, ExportExcel.CELL_STYLE_NAME_DATA, totalExpectCharge);
                ExportExcel.createCell(dataRow, 19, xStyle, ExportExcel.CELL_STYLE_NAME_DATA, totalBlockCharge);

                ExportExcel.createCell(dataRow, 20, xStyle, ExportExcel.CELL_STYLE_NAME_HEADER, "");
                xSheet.addMergedRegion(new CellRangeAddress(dataRow.getRowNum(), dataRow.getRowNum(), 20, 21));
            }

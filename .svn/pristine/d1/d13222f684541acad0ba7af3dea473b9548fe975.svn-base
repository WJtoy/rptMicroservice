...
public class CustomerPraiseDetailsRptService extends RptBaseService {
    ...
    public Page<RPTKeFuPraiseDetailsEntity> getCustomerPraiseDetailsList(RPTKeFuCompleteTimeSearch search) {
        ...
        List<String> customerFields = Lists.newArrayList("id", "name","salesId");
        Map<Long, RPTCustomer> customerMap  = msCustomerService.getCustomerMapWithCustomizeFields(customerIds, customerFields);
        List<Long> salesIds = customerMap.values().stream()
                .filter(i -> i.getSales() != null && i.getSales().getId() != null)
                .map(i -> i.getSales().getId())
                .distinct()
                .collect(Collectors.toList());
        Map<Long, String> namesByUserIds = MSUserUtils.getNamesByUserIds(salesIds);
        ...
        for(RPTKeFuPraiseDetailsEntity item : page){
            ...
            */
            item.setAreaName(rptArea == null?StringUtils.EMPTY:rptArea.getFullName());
        }
        return page;
    }

...
@Slf4j
@Component
public class RPTRebuildMiddleTableTaskMQSender implements RabbitTemplate.ConfirmCallback {
    ...
    @Autowired
    public RPTRebuildMiddleTableTaskMQSender(RabbitTemplate kklRabbitTemplate, RetryTemplate kklRabbitRetryTemplate) {
     ...
     * @param message 消息体
     */
    public boolean send(MQRPTRebuildMiddleTableTaskMessage.RPTRebuildMiddleTableTaskMessage message) {
        ...
        try {
            retryTemplate.execute((RetryCallback<Object, Exception>) context -> {
                ...
                rabbitTemplate.convertAndSend(
                        RPTMQConstant.MQ_RPT_REBUILD_MIDDLE_TABLE_TASK,
                        RPTMQConstant.MQ_RPT_REBUILD_MIDDLE_TABLE_TASK,
                        message.toByteArray(),
                        msg -> {
                ...
                MQRPTRebuildMiddleTableTaskMessage.RPTRebuildMiddleTableTaskMessage msg = MQRPTRebuildMiddleTableTaskMessage.RPTRebuildMiddleTableTaskMessage.parseFrom((byte[]) msgObj);
                Throwable throwable = context.getLastThrowable();
                log.error("normal send error {}, {}", throwable.getLocalizedMessage(), msg);
                return null;
            });
        } catch (Exception e) {
            log.error("RPTRebuildMiddleTableTaskMQSender.send：{}", e.getLocalizedMessage());
        }
        return result.get();
    }

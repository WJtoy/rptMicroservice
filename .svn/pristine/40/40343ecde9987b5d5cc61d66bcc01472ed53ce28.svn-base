...
public class CtCustomerOrderDailyRptService extends RptBaseService {
     ...
     * 获取客户每日下单列表
     */
    public Page<RPTCustomerNewOrderDailyRptEntity> getCustomerNewOrderDailyList(RPTSearchCondtion rptSearchCondtion) {
...
//            entity.setOrderItems(RPTOrderItemUtils.fromOrderItemsJson(entity.getOrderItemJson()));
     ...
     * 创建Excel表格
     */
    public SXSSFWorkbook exportCustomerNewOrderDailyRpt(String searchConditionJson, String reportTitle) {
        SXSSFWorkbook xBook = null;
        try {
            RPTSearchCondtion searchCondtion = redisGsonService.fromJson(searchConditionJson, RPTSearchCondtion.class);
            List<RPTCustomerNewOrderDailyRptEntity> list = getCustomerNewOrderDailyList(searchCondtion);
            ...
            xSheet.addMergedRegion(new CellRangeAddress(titleRow.getRowNum(), titleRow.getRowNum(), 0, 16));
            ...
            ExportExcel.createCell(headerSecondRow, 4, xStyle, ExportExcel.CELL_STYLE_NAME_HEADER, "下单人");
            ...
            for (RPTCustomerNewOrderDailyRptEntity orderMaster : list) {
                ...
                ExportExcel.createCell(dataRow, 13, xStyle, ExportExcel.CELL_STYLE_NAME_DATA, (orderMaster.getServicePhone()));
                if (rowSpan > 0) {
                    xSheet.addMergedRegion(new CellRangeAddress(dataRow.getRowNum(), dataRow.getRowNum() + rowSpan, 13, 13));
                }
                ExportExcel.createCell(dataRow, 14, xStyle, ExportExcel.CELL_STYLE_NAME_DATA, (orderMaster.getFullServiceAddress()));
                if (rowSpan > 0) {
                    xSheet.addMergedRegion(new CellRangeAddress(dataRow.getRowNum(), dataRow.getRowNum() + rowSpan, 14, 14));
                }
                ExportExcel.createCell(dataRow, 15, xStyle, ExportExcel.CELL_STYLE_NAME_DATA,
            ...
            ExportExcel.createCell(dataRow, 9, xStyle, ExportExcel.CELL_STYLE_NAME_DATA, totalCount);
            ExportExcel.createCell(dataRow, 10, xStyle, ExportExcel.CELL_STYLE_NAME_DATA, totalBlocked);
            ExportExcel.createCell(dataRow, 11, xStyle, ExportExcel.CELL_STYLE_NAME_HEADER, "");
            xSheet.addMergedRegion(new CellRangeAddress(dataRow.getRowNum(), dataRow.getRowNum(), 11, 16));
        } catch (Exception e) {
            log.error("客户每日下单明细报表写入excel失败:{}", Exceptions.getStackTraceAsString(e));
            return null;
        }
        return xBook;
    }

...
public class ExploitDetailRptService extends RptBaseService {
           ...
           // Map<Long, RPTCustomer> customerMap  = msCustomerService.getCustomerMapWithCustomizeFields(Lists.newArrayList(customerIds), Arrays.asList(fieldsArray));
            for (RPTExploitDetailEntity entity : returnPage) {
               // entity.setCustomerName(customerMap.get(entity.getCustomerId()).getName());
                ...
                int minutes = (int) (entity.getTimeLiness()*60);
    ...
    public SXSSFWorkbook exploitDetailExport(String searchConditionJson, String reportTitle) {
        ...
        try {
            ...
            ExportExcel.createCell(headerFirstRow, 9, xStyle, ExportExcel.CELL_STYLE_NAME_HEADER, "用户名");
            xSheet.addMergedRegion(new CellRangeAddress(headerFirstRow.getRowNum(), headerFirstRow.getRowNum() + 1, 9, 9));
            ExportExcel.createCell(headerFirstRow, 10, xStyle, ExportExcel.CELL_STYLE_NAME_HEADER, "用户电话");
            xSheet.addMergedRegion(new CellRangeAddress(headerFirstRow.getRowNum(), headerFirstRow.getRowNum() + 1, 10, 10));
            ExportExcel.createCell(headerFirstRow, 11, xStyle, ExportExcel.CELL_STYLE_NAME_HEADER, "省市区");
            xSheet.addMergedRegion(new CellRangeAddress(headerFirstRow.getRowNum(), headerFirstRow.getRowNum() + 1, 11, 11));
            ExportExcel.createCell(headerFirstRow, 12, xStyle, ExportExcel.CELL_STYLE_NAME_HEADER, "地址");
            xSheet.addMergedRegion(new CellRangeAddress(headerFirstRow.getRowNum(), headerFirstRow.getRowNum() + 1, 12, 12));
            ExportExcel.createCell(headerFirstRow, 13, xStyle, ExportExcel.CELL_STYLE_NAME_HEADER, "突击次数");
            xSheet.addMergedRegion(new CellRangeAddress(headerFirstRow.getRowNum(), headerFirstRow.getRowNum() + 1, 13, 13));
            ExportExcel.createCell(headerFirstRow, 14, xStyle, ExportExcel.CELL_STYLE_NAME_HEADER, "完成时效");
            xSheet.addMergedRegion(new CellRangeAddress(headerFirstRow.getRowNum(), headerFirstRow.getRowNum() + 1, 14, 14));
            ExportExcel.createCell(headerFirstRow, 15, xStyle, ExportExcel.CELL_STYLE_NAME_HEADER, "加费用");
            xSheet.addMergedRegion(new CellRangeAddress(headerFirstRow.getRowNum(), headerFirstRow.getRowNum(), 15, 16));

            //表头第二行
            Row headerSecondRow = xSheet.createRow(rowIndex++);
            headerSecondRow.setHeightInPoints(EXECL_CELL_HEIGHT_HEADER);

            ExportExcel.createCell(headerSecondRow, 15, xStyle, ExportExcel.CELL_STYLE_NAME_HEADER, "远程");
            ExportExcel.createCell(headerSecondRow, 16, xStyle, ExportExcel.CELL_STYLE_NAME_HEADER, "其他");
            ...
            if (list != null) {
                for (int i = 0; i < list.size(); i++) {
                   ...
                   // ExportExcel.createCell(dataRow, 9, xStyle, ExportExcel.CELL_STYLE_NAME_DATA, entity.getCustomerName() == null ? "" : entity.getCustomerName());
                    ExportExcel.createCell(dataRow, 9, xStyle, ExportExcel.CELL_STYLE_NAME_DATA, entity.getUserName() == null ? "" : entity.getUserName());
                    ExportExcel.createCell(dataRow, 10, xStyle, ExportExcel.CELL_STYLE_NAME_DATA, entity.getUserPhone() == null ? "" : entity.getUserPhone());
                    ExportExcel.createCell(dataRow, 11, xStyle, ExportExcel.CELL_STYLE_NAME_DATA, entity.getAreaName() == null ? "" : entity.getAreaName());
                    ExportExcel.createCell(dataRow, 12, xStyle, ExportExcel.CELL_STYLE_NAME_DATA, entity.getUserAddress() == null ? "" : entity.getUserAddress());
                    ExportExcel.createCell(dataRow, 13, xStyle, ExportExcel.CELL_STYLE_NAME_DATA, "第"+entity.getItemNo()+"次突击");
                    ExportExcel.createCell(dataRow, 14, xStyle, ExportExcel.CELL_STYLE_NAME_DATA, entity.getTimeLinessLabel()==null?"":entity.getTimeLinessLabel());

                    ExportExcel.createCell(dataRow, 15, xStyle, ExportExcel.CELL_STYLE_NAME_DATA, entity.getEngineerTravelCharge());
                    ExportExcel.createCell(dataRow, 16, xStyle, ExportExcel.CELL_STYLE_NAME_DATA, entity.getEngineerOtherCharge());
                ...
                xSheet.addMergedRegion(new CellRangeAddress(dataRow.getRowNum(), dataRow.getRowNum(), 0, 14));


                ExportExcel.createCell(dataRow, 15, xStyle, ExportExcel.CELL_STYLE_NAME_DATA, engineerTravelCharge);
                ExportExcel.createCell(dataRow, 16, xStyle, ExportExcel.CELL_STYLE_NAME_DATA, engineerOtherCharge);

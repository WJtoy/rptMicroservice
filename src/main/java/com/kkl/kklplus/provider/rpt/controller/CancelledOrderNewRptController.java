...
import com.kkl.kklplus.provider.rpt.service.CancelledOrderNewRptService;
...
@RestController
@RequestMapping("cancelledOrderNew")
public class CancelledOrderNewRptController {
    @Autowired
     ...
     * 分页&条件查询
     */
    @ApiOperation("分页获取每日退单或取消工单明细")
    @PostMapping("getCancelledOrderNewList")
    public MSResponse<MSPage<RPTCancelledOrderEntity>> getCancelledOrderList(@RequestBody RPTCancelledOrderSearch search) {
        Page<RPTCancelledOrderEntity> pageList = cancelledOrderNewRptService.getCancelledOrderListByPaging(search);
